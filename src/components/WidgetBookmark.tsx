  ...
  return (
    <div className="flex flex-col pt-5 pb-6 px-3 sm:px-0">
      ...
      <div className="rounded-lg p-2 bg-[#ffffff54] backdrop-blur-sm shadow-[0_-5px_15px_2px_rgba(0,0,0,0.07)">
        <div className="border-dashed border-2 rounded-lg w-full h-full flex flex-col justify-center items-center p-[9px]">
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-[6px] w-full">
              {bookmark.map((item: BookmarkObject, key: number) => (
                ...
                >
                  <div className="text-center text-xs sm:text-sm text-[#2F6742] font-arabic">{item.arabic}</div>
                  <div className="text-center text-xs sm:text-sm text-gray-700 font-bold">{item.name}</div>
                  <div className="text-center text-xs sm:text-xs text-gray-400">ayat {item.ayat}</div>
                </Link>
              ))}
            </div>
          ) : (
            <span className="text-xs text-[#0000004d] py-5">Belum ada ayat yang ditandai</span>
          )}
        </div>
      </div>
    </div>
